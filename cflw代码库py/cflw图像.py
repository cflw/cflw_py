...
#===============================================================================
#像素
def f双像素运算m(a像素1: tuple, a像素2: tuple, af运算s):
	return tuple(af运算s(v像素1, v像素2) for v像素1, v像素2 in zip(a像素1, a像素2))
def F双像素运算m(a像素1: tuple, a像素2: tuple, af运算s):
...
def f像素读s(a像素: int, a掩码: int):
	"单通道"
	return a像素 & a掩码
def f像素放大m(a像素: tuple, a掩码: int):
	"多通道"
	return tuple(f像素放大s(v像素, a掩码) for v像素 in a像素)
def f像素放大s(a像素: int, a掩码: int):
	"单通道"
	return a像素 * 255 // a掩码
#===============================================================================
# 二维图像计算
...
def f单图像像素运算(a图像, af运算):
	v图像数据 = list(a图像.getdata())
	v处理数据 = list(af运算(v图像像素) for v图像像素 in v图像数据)
	return fc图像(v处理数据, a图像.mode, a图像.size)
def f双图像像素运算_顺序(a图像0, a图像1, af运算):
	"""逐数据运算,如果图像尺寸不同,运算的像素可能有对不上"""
	v图像数据0 = list(a图像0.getdata())
	v图像数据1 = list(a图像1.getdata())
	v处理数据 = list(af运算(v图像像素0, v图像像素1) for v图像像素0, v图像像素1 in zip(v图像数据0, v图像数据1))
	return fc图像(v处理数据, a图像0.mode, a图像0.size)
def f双图像像素运算_对齐(a图像0, a图像1, af运算):
	...
	def f运算(x, y):
		if x <= v宽1 and y <= v高1:
			return af运算(v图像数据0[vf索引0(x, y)], v图像数据1[vf索引1(x, y)])
		else:
...
def f添加最低有效位水印(a图像, a水印, a掩码 = c水印掩码):
	return f双图像像素运算_顺序(a图像, a水印, lambda a像素, a写: f像素写m(a像素, a写, a掩码))
def f提取最低有效位水印(a图像, a掩码 = c水印掩码):
	return f单图像像素运算(a图像, lambda a像素: f像素放大m(f像素读m(a像素, a掩码), a掩码))
def f最低有效位校验(a图像, a掩码 = c水印掩码):
	return Image.eval(a图像, functools.partial(f像素放大s, a掩码 = c水印掩码))
