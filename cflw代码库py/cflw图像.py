...
# 混合,像素必需是rgba字节整数格式
#===============================================================================
def f颜色混合_正常(c0: int, a0: int, c1: int, a1: int):
	return c0 * (255 - a1) // 255 + c1 * a1 // 255
def f颜色混合_叠加(c0: int, a0: int, c1: int, a1: int):
	return (c0 * a0 // 255 + c1 * a1 // 255) % 255
def f颜色混合_叠底(c0: int, a0: int, c1: int, a1: int):
	return (c0 * a0 // 255 - c1 * a1 // 255) % 255
def f像素混合(a像素0: tuple, a像素1: tuple, af颜色通道, af阿通道):
	a0 = a像素0[3]
	a1 = a像素1[3]
	r = af颜色通道(a像素0[0], a0, a像素1[0], a1)
	g = af颜色通道(a像素0[1], a0, a像素1[1], a1)
	b = af颜色通道(a像素0[2], a0, a像素1[2], a1)
	a = af阿通道(a0, a1)
	return (r, g, b, a)
def f像素混合_正常(a像素0: tuple, a像素1: tuple):
	return f像素混合(a像素0, a像素1, f颜色混合_正常, max)
def f像素混合_叠加(a像素0: tuple, a像素1: tuple):
	return f像素混合(a像素0, a像素1, f颜色混合_叠加, max)
def f像素混合_叠底(a像素0: tuple, a像素1: tuple):
	return f像素混合(a像素0, a像素1, f颜色混合_叠底, max)
#===============================================================================
# 通用图像处理
#===============================================================================
...
def f平铺图像(a图像, a尺寸):
	...
	v图像 = Image.new(a图像.mode, a尺寸, (0, 0, 0, 0))
	v尺寸x, v尺寸y = ft尺寸(a尺寸)
...
def f添加透明水印(a图像, a水印, a透明度 = c水印透明度):
	"""PIL.Image.blend不处理透明通道,所以写一个替代"""
	def f混合(a像素0: tuple, a像素1: tuple):
		a0 = a像素0[3] if len(a像素0) > 3 else 1
		a1 = int(a像素1[3] * a透明度)
		r = f颜色混合_正常(a像素0[0], a0, a像素1[0], a1)
		g = f颜色混合_正常(a像素0[1], a0, a像素1[1], a1)
		b = f颜色混合_正常(a像素0[2], a0, a像素1[2], a1)
		a = max(a0, a1)
		return (r, g, b, a)
	return f双图像像素运算_顺序(a图像, a水印, f混合)