...
def fi标识符字符(c: str):
	"""可以作为标识符字符的:
	数字, 大小写字母, _, 各国语言文字(除了标点符号)
	"""
	v编码 = ord(c)
	if v编码 < 0x30:	#符号
		return False
	elif v编码 < 0x3a:	#0x30~0x3a 数字
		return True
	elif v编码 < 0x41:	#符号
		return False
	elif v编码 < 0x5b:	#0x41~0x5a 大写字母
		return True
	elif v编码 == 0x5f:	#0x5f 下划线
		return True
	elif v编码 < 0x61:	#符号
		return False
	elif v编码 < 0x7b:	#0x61~0x7b 小写字母
		return True
	elif v编码 < 0x2e80:
		return False
	elif v编码 < 0x9fa5:	#中日韩符号文字
		return True
	else:
		return False
def f去特殊字符(s: str):	#
	"""去除不能作为python标识符的字符"""
	return "".join(list(filter(lambda c: fi标识符字符(c), s)))
#===============================================================================
# 字符串转换
#===============================================================================
def ft字符串(*a元组, a分隔符 = "\t"):
