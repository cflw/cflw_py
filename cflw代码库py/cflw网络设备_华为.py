...
class C系统视图(设备.I全局配置模式):
	...
	def fs设备名(self, a名称):
		self.f切换到当前模式()
		self.m设备.f执行命令('sysname ' + str(a名称))
#==============================================================================
# 接口
#==============================================================================
...
# 开放最短路径优先
#==============================================================================
class C开放最短路径优先(设备.I开放最短路径优先):
	def __init__(self, a, a进程号):
		设备.I开放最短路径优先.__init__(self, a, a进程号)
		self.m区域表 = {}
	def fg模式参数(self):
		s = str(self.m进程号)
		if hasattr(self, 'm路由器号'):
			s += ' router-id ' + str(self.m路由器号)
		return s
	def fg进入命令(self):
		return '开放最短路径优先 ' + self.fg模式参数()
	def f模式_区域(self, a):
		v区域 = C开放最短路径优先.f计算区域号(a)
		if not v区域 in self.m区域表:
			self.m区域表[v区域] = C开放最短路径优先区域(self, v区域)
	...
	#显示
	def f显示_路由表(self):
		return self.m设备.f执行显示命令('display ip routing-table protocol 开放最短路径优先')
	#操作
	def fs路由器号(self, a):
		self.m路由器号 = a
...
class C开放最短路径优先区域(设备.I模式):
	...
	def f通告网络(self, a网络号, a区域):
		v命令 = C开放最短路径优先区域.f通告网络命令(a网络号, a区域)
		self.f切换到当前模式()
		self.m设备.f执行命令(v命令)
	def f删除网络(self, a网络号, a区域):
		v命令 = 'undo' + C开放最短路径优先区域.f通告网络命令(a网络号, a区域)
		self.f切换到当前模式()
		self.m设备.f执行命令(v命令)
#==============================================================================
# 工具
