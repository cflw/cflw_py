...
class C区域(设备.I开放最短路径优先区域, 设备.C同级模式):
	...
	def fe行(self):
		for v行 in 字符串.fe分割(self.m字符串, "\n"):
			if v行.count(".") != 6:	#2个地址共6个点
				continue
			v邻居s, v优先级s, v状态s, v死亡s, v地址s, v接口s = 字符串.fe按位置分割(v行, C邻居表.c邻居开始, C邻居表.c优先级开始, C邻居表.c状态开始, C邻居表.c死亡时间开始, C邻居表.c地址开始, C邻居表.c接口开始)
