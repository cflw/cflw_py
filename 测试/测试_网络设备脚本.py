...
def main():
	...
	#v设备 = 华为.f创建设备(v连接, 华为.E型号.ar201, 5.13)
	#v设备.f自动适应延迟()
	v设备.fs回显(True)
	#for i in range(5):
	...
	# except 设备.X命令异常 as e:
	# 	print("捕获异常")
	v用户模式.f显示_时间()
	v全局配置 = v用户模式.f模式_全局配置()
	v接口配置 = v全局配置.f模式_接口配置("f0/1")
	#v接口配置 = v全局配置.f模式_接口配置("e0/0/1")
	...
	#v访问列表.f添加规则(2, 设备.C访问控制列表.S规则(a允许 = False, a协议 = 设备.C访问控制列表.E协议.tcp, a源地址 = ipaddress.IPv4Network("1.2.3.4/24", False), a源端口 = 1, a目的端口 = range(100, 1000)))
